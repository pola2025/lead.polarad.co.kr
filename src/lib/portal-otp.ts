/**
 * 클라이언트 포털 텔레그램 OTP 인증
 *
 * 클라이언트의 telegramChatId로 OTP 전송 (리드 알림과 동일한 채팅)
 * TELEGRAM_BOT_TOKEN 환경변수 재활용
 */

const OTP_EXPIRY_MS = 5 * 60 * 1000; // 5분
const OTP_LENGTH = 6;
const MAX_ATTEMPTS = 5;
const LOCKOUT_MS = 60 * 1000; // 1분 잠금
const RESEND_COOLDOWN_MS = 30 * 1000; // 재발송 쿨다운 30초

interface OtpEntry {
  code: string;
  expiresAt: number;
  attempts: number;
  lockedUntil?: number;
}

// 슬러그별 OTP 저장소
const portalOtpStore = new Map<string, OtpEntry>();

function generateOTP(): string {
  const array = new Uint32Array(1);
  crypto.getRandomValues(array);
  return String(array[0] % 1000000).padStart(OTP_LENGTH, "0");
}

async function sendViaTelegram(
  chatId: string,
  message: string,
): Promise<boolean> {
  const botToken = process.env.TELEGRAM_BOT_TOKEN;
  if (!botToken) {
    console.error("Portal OTP: TELEGRAM_BOT_TOKEN 미설정");
    return false;
  }

  try {
    const res = await fetch(
      `https://api.telegram.org/bot${botToken}/sendMessage`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          chat_id: chatId,
          text: message,
          parse_mode: "HTML",
        }),
      },
    );
    if (!res.ok) {
      const err = await res.json();
      console.error("Portal OTP 전송 실패:", err);
      return false;
    }
    return true;
  } catch (error) {
    console.error("Portal OTP 전송 오류:", error);
    return false;
  }
}

/** OTP 생성 및 클라이언트 텔레그램으로 전송 */
export async function sendPortalOTP(
  slug: string,
  clientName: string,
  telegramChatId: string,
): Promise<{ success: boolean; error?: string }> {
  const now = Date.now();
  const existing = portalOtpStore.get(slug);

  // 재발송 쿨다운 체크
  if (
    existing &&
    existing.expiresAt - OTP_EXPIRY_MS + RESEND_COOLDOWN_MS > now
  ) {
    return { success: false, error: "잠시 후 다시 시도해주세요. (30초 대기)" };
  }

  const code = generateOTP();
  portalOtpStore.set(slug, {
    code,
    expiresAt: now + OTP_EXPIRY_MS,
    attempts: 0,
  });

  const message = `
<b>🔑 ${clientName} 포털 인증코드</b>

<b>인증코드:</b> <code>${code}</code>
<b>유효시간:</b> 5분

⏰ ${new Date().toLocaleString("ko-KR", { timeZone: "Asia/Seoul" })}
`.trim();

  const sent = await sendViaTelegram(telegramChatId, message);
  if (!sent) {
    portalOtpStore.delete(slug);
    return {
      success: false,
      error: "인증코드 발송에 실패했습니다. 텔레그램 채팅 ID를 확인해주세요.",
    };
  }

  return { success: true };
}

/** OTP 검증 */
export function verifyPortalOTP(
  slug: string,
  code: string,
): { valid: boolean; error?: string; lockedUntil?: number } {
  const now = Date.now();
  const entry = portalOtpStore.get(slug);

  if (!entry) {
    return { valid: false, error: "인증코드를 먼저 요청해주세요." };
  }

  // 잠금 상태 체크
  if (entry.lockedUntil && now < entry.lockedUntil) {
    const remaining = Math.ceil((entry.lockedUntil - now) / 1000);
    return {
      valid: false,
      error: `너무 많은 시도입니다. ${remaining}초 후 다시 시도해주세요.`,
      lockedUntil: entry.lockedUntil,
    };
  }

  // 만료 체크
  if (now > entry.expiresAt) {
    portalOtpStore.delete(slug);
    return {
      valid: false,
      error: "인증코드가 만료되었습니다. 다시 요청해주세요.",
    };
  }

  entry.attempts++;

  // 실패 횟수 초과 → 1분 잠금
  if (entry.attempts >= MAX_ATTEMPTS) {
    entry.lockedUntil = now + LOCKOUT_MS;
    entry.attempts = 0;
    return {
      valid: false,
      error: "인증 시도 횟수를 초과했습니다. 1분 후 다시 시도해주세요.",
      lockedUntil: entry.lockedUntil,
    };
  }

  if (entry.code !== code) {
    return {
      valid: false,
      error: `인증코드가 올바르지 않습니다. (${MAX_ATTEMPTS - entry.attempts}회 남음)`,
    };
  }

  portalOtpStore.delete(slug);
  return { valid: true };
}
