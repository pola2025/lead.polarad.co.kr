 ...
 * 관리자 텔레그램 OTP 인증
 *
 * 기존 TELEGRAM_BOT_TOKEN + TELEGRAM_CHAT_ID 환경변수 재활용
 */

const OTP_EXPIRY_MS = 5 * 60 * 1000; // 5분
const OTP_LENGTH = 6;
const MAX_ATTEMPTS = 5;
const LOCKOUT_MS = 60 * 1000; // 실패 초과 시 1분 재시도 대기
...
export function verifyAdminOTP(code: string): {
  ...
  if (entry.attempts >= MAX_ATTEMPTS) {
    ...
    entry.attempts = 0; // 잠금 해제 후 재시도 허용
    return {
      valid: false,
